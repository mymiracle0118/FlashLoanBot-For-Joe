...
const web3 = new Web3(
  new Web3.providers.WebsocketProvider(process.env.ETH_WSS, {
    reconnect: {
      auto: true,
      delay: 5000,
      maxAttempts: 15,
...
const getPrices = async() => {
  ...
  try {
    ...
    prices[process.env.ERC20_USDT_MAINNET.toLowerCase()] = json.tether.usd;
  } catch (e) {
    console.error(e);
    return {};
  }
...
const init = async () => {
  ...
  const transactionSender = TransactionSender.factory(process.env.ETH_WSS.split(','));
    ...
    pairs.forEach((pair) => {
      calls.push(async () => {
        ...
        if (profit > 0) {
          ...
          let gasCostUsd = (txCostETH / 1e18) * prices[process.env.WETH_MAINNET.toLowerCase()];
